...
export const PURCHASE_COLUMN_KEYS = [
  'entryNumber','contractSubject','supplierName','smp','supplierInn',
  'initialPrice','purchaseAmount','contractNumber','contractDate','validFrom',
  'validTo','contractEnd','placementDate','methodOfPurchase','documentNumber',
  'completed','savings','performanceAmount','performanceForm',
  'additionalAgreementNumber','currentContractAmount','publication',
  'responsible','planNumber','applicationAmount','comment','actions',
] as const;

export function buildPurchaseColumns(onEdit: (p: Purchase) => void): ColumnsType<Purchase> {
  return [
    ...
    { title: 'Состоялась', dataIndex: 'completed', key: 'completed', width: 130, sorter: true, render: (v) => (v ? <Tag color="green">Да</Tag> : <Tag>Нет</Tag>) },
    ...
    { title: 'Примечания', dataIndex: 'comment', key: 'comment', width: 240, ellipsis: true },
    {
      ...
      render: (_: any, record: Purchase) => (
        <Button type="link" onClick={() => onEdit(record)}>Изменить</Button>
      ),
    },
  ];
}

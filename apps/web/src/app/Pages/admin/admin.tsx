...
type BatchResponse = { inserted?: number; upserted?: number; modified?: number; matched?: number };
...
// Точные соответствия по вашим формулировкам
...
async function postBatch(
  ...
  const url = new URL('purchases/batch', BASE_URL.endsWith('/') ? BASE_URL : BASE_URL + '/');
  ...
  if (!res.ok) {
    ...
    throw new Error(`Ошибка загрузки (${res.status}): ${txt || res.statusText}`);
  }
  return res.json();
}
...
export default function AdminPage() {
  ...
  const sample = useMemo(() => JSON.stringify(items.slice(0, 3), null, 2), [items]);
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      return postBatch(items, mode, mode === 'upsert' ? matchBy : undefined, abortRef.current.signal);
    },
    onSuccess: (res) => {
      setInfo(
        mode === 'insert'
          ? `Готово. Вставлено: ${res.inserted ?? 0}`
    : `Готово. Upsert: ${res.upserted ?? 0}, Обновлено: ${res.modified ?? 0}, Совпало: ${res.matched ?? 0}`
    );
    },
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const list = rawRows.map(mapRow).filter((row) => Object.keys(row).length > 0);
      setItems(list);
  ...
  function handleClear() {
    setItems([]); setFileName(''); setError(null); setInfo(null);
  }
    ...
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto' }}>
      ...
      <section style={{ marginTop: 16, padding: 16, border: '1px solid #eee', borderRadius: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          <input type="file" accept=".xlsx,.xls" onChange={handleFile} disabled={mutation.isPending} />
          <button type="button" onClick={handleClear} disabled={mutation.isPending}>Очистить</button>
          {fileName ? <span style={{ color: '#666' }}>{fileName}</span> : null}
        </div>
        <div style={{ marginTop: 8, color: '#666' }}>
          Ожидаются колонки в точности как в вашей таблице. Небольшие вариации пробелов/регистра поддерживаются.
        </div>
      </section>
      ...
      <section style={{ marginTop: 16, padding: 16, border: '1px solid #eee', borderRadius: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap' }}>
          <label>
            ...
            <select value={mode} onChange={(e) => setMode(e.target.value as Mode)} disabled={mutation.isPending}>
      ...
      <section style={{ marginTop: 16, padding: 16, border: '1px solid #eee', borderRadius: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <button type="button" onClick={() => mutation.mutate()} disabled={!items.length || mutation.isPending}>
            Отправить на сервер
          </button>
        ...
        <div style={{ marginTop: 12 }}>
          <div>Подготовлено записей: <b>{items.length}</b></div>
          {items.length > 0 && (
            <>
              <div style={{ marginTop: 8, color: '#666' }}>Пример первых записей</div>
              <pre style={{ background: '#fafafa', border: '1px solid #eee', borderRadius: 6, padding: 12, overflow: 'auto', maxHeight: 220 }}>
            {sample}
          </pre>
            </>
          )}
        ...
        {mutation.isPending && <div style={{ marginTop: 12, color: '#555' }}>Отправка...</div>}

        {mutation.isSuccess && (
          <div style={{ marginTop: 12, color: 'green' }}>
            {mode === 'insert'
              ? `Готово. Вставлено: ${mutation.data?.inserted ?? 0}`
              : `Готово. Upsert: ${mutation.data?.upserted ?? 0}, Обновлено: ${mutation.data?.modified ?? 0}, Совпало: ${mutation.data?.matched ?? 0}`}
          </div>
        )}
